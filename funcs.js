...
module.exports = bot => {
  ...
  bot.promptForUpvote = function(msg, command) {
    msg.channel.send(
      `To use the **${command}** command, please go upvote me on discordbots.org! ` +
  ...
  bot.getPrefix = function(msg) {
    return new Promise((resolve, reject) => {
      db.all(
        `SELECT * FROM servers WHERE id = "${msg.guild.id}"`,
        (err, rows) => {
          if (err || !rows[0]) reject(err);
  ...
  bot.setPrefix = function(prefix, guild) {
    db.run(
      'UPDATE servers SET prefix = "' + prefix + '" WHERE id = ' + guild.id
    );
    return prefix;
  };
    ...
    if (afk.length !== 0) {
      for (let i = 0; i < afk.length; i++) {
        ...
        if (msg.mentions.users.size > 0 && afk.length !== 0) {
          if (
            msg.content.indexOf(afk[i].id) !== -1 &&
            msg.author.id !== afk[i].id
          ) {
      ...
      this.getCurrentBooleanSetting("mentionSpamProtection", msg.guild.id).then(
        setting => {
          if (setting && msg.deletable) {
            msg.delete().then(() => {
              msg.reply(
                "don't mention more than 4 people at once! :no_entry_sign:"
              );
            });
          } else if (setting && !msg.deletable) {
            msg.guild.owner.send(
